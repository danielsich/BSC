...
#append results
def append_results_to_csv(customers, averagespeed, distance, vehicles, costs, tts, weighted_load, positive_height_diff,total_fuel, driver_pay, filepath='output/outprpsize10.csv'):
...
def append_nan_results_to_csv(customers, filepath='output/outprpsize10.csv'):
    with open(filepath, 'a', newline='') as csvfile:
        csvwriter = csv.writer(csvfile)
...
for xxx in range(5,51):
    for a in range(10):
        ...
        zfprp = ((quicksum(a_ij[i, j] * Dist[i, j] * W * x[i, j] for i in N for j in N) + quicksum(a_ij[i, j] * f[i, j] * Dist[i, j] for i in N for j in N) + quicksum(
            ...
            in N))/(eff * H * enn))*cfe + quicksum(p * s[j] * x[j, 0] for j in N0)
